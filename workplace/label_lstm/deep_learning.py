...
def rerun_get_model():
    ...
    # 预测数据
    pre_df = pd.DataFrame(columns=['store_id', 'name', 'category3_new', 'predict_category'])
    for i in range(SP.SEGMENT_NUMBER):
        result_df = predict_result(lstm_model, prepro, i)
        pre_df = pd.concat((pre_df, result_df))
    pre_df.to_csv(SP.PATH_ZZX_PREDICT_DATA + 'predict_category_' + 'hb_all' + '.csv')
