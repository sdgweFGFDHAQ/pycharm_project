...
# 读取原始文件,将数据格式标准化
def set_file_standard_data(city):
    csv_data = pd.read_csv(SP.PATH_ZZX_DATA + city + '.csv',
    ...
    csv_data.to_csv(SP.PATH_ZZX_STANDARD_DATA + 'standard_store_' + city + '.csv',
                    columns=['id', 'name', 'category3_new', 'cut_name'])


def get_dataset():
    gz_df = pd.read_csv('../standard_store_gz.csv')
...
def predict_result(tokenizer, model, id_cat_dict, city, part_i):
    try:
...
if __name__ == '__main__':
    ...
    #     path = SP.PATH_ZZX_PREDICT_DATA + 'predict_category_' + str(csv_i) + '.csv'
    ...
    # cities = ['洛阳市', '周口市', '云浮市', '珠海市', '三门峡市', '鹤壁市', '信阳市', '佛山市', '梅州市', '濮阳市',
    #           '徐州市', '宿迁市', '无锡市', '盐城市', '泰州市', '齐齐哈尔市', '常州市', '黑河市', '汉中市', '宝鸡市',
    #           '安康市', '西安市', '咸阳市', '商洛市','玉树藏族自治州', '海东市','巴中市', '辽源市', '延边朝鲜族自治州']
    city_num = len(cities)
    ...
    for i in range(len(index_list) - 1):
        ...
        for city in cities_i:
            set_file_standard_data(city)
            # predict_result(tokenizer, model, id_cat_dict, city, i)
