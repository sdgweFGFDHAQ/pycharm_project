...
# 输入模型的样本数
input_examples = 50000
output_file = './dataset/label_map.txt'
# 词汇表词个数
vocabulary_size = 70000
# 不在词汇表的额外词汇
extra_vocab = 200
# 字典
label_map_file = './dataset/label_map.txt'
...
def _added_token_counts(data_iterator, try_swapping, max_input_examples=10000):
    ...
    for sources, target in data_iterator:
        ...
        if len(source_merge) > max_seq_length:
            print("max_seq_length=%d, len(source_merge)=%d,source_merge:%s" %
...
def _construct_added_phrases_matrix(all_added_phrases, phrase_counter):
    ...
    matrix = scipy.sparse.dok_matrix((len(all_added_phrases), len(phrase_2_idx)), dtype=bool)
...
def phrase_vocabulary():
    ...
    with tf.io.gfile.GFile(output_file, 'w') as writer:
        with tf.io.gfile.GFile(statistics_file, 'w') as stats_writer:
            ...
            writer.write('KEEP\n')
            writer.write('DELETE\n')
            for i, (phrase, count) in enumerate(phrase_counter.most_common(vocabulary_size + extra_vocab)):
                # Write tags.
    ...
    logging.info(f'Wrote tags to: {output_file}')
