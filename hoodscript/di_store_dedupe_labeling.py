...
# 清空目标表数据
table_name = 'di_store_dedupe_labeling'
cm = pySparkCm(table_name)
spark = cm.sparkenv()
spark.sql("truncate table standard_db.di_store_dedupe_labeling")
print("table truncate complete")

# 批量读取数据写入到表
# csv数据分为8段
for csv_i in range(8):
    # 远程连接服务器读取文件
    ssh = SSH()
    connect = ssh.connect()
    ...
    cm.write_to_hudi(df, 'standard_db', 'di_store_dedupe_labeling', 'store_id', '', 'store_id', 'append', 'insert')
    print("Complete!")
